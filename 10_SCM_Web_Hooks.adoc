...
* If you are using a lab hosted in the public cloud, we will fork the repository from the previous lab which was here https://github.com/RedHatWorkshops/bluegreen. For OCP instances which cannot be accessed on the internet, simply use the repo from the previous lab in the command below.
